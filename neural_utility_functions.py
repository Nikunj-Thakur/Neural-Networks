...
def plt_linear(X_train, Y_train, prediction_tf, prediction_np):
    ...
    ax[0].plot(X_train, prediction_tf, c=model_color, linewidth=2.5, label="Model output")
    ax[0].text(1.6, 350, r"y=$200 x + 100$", fontsize='xx-large', color=text_color)
    ...
    ax[1].plot(X_train, prediction_np, c=model_color, linewidth=2.5, label="Model output")
    ax[1].text(1.6, 350, r"y=$200 x + 100$", fontsize='xx-large', color=text_color)
...
def plt_logistic(X_train, Y_train, model, set_w, set_b, pos, neg):
    ...
    layerf= lambda x : sigmoid(np.dot(set_w,x.reshape(1,1)) + set_b)
